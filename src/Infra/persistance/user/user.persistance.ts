...
export class UserPersistance implements UserRepository
{
    ...
    async create(user: IUser): Promise<void>
    {
        ...
        {
            const db = this.databaseService.getDb();
            const { user_id, name, email, password_hash, role, is_verified, created_at } = user;
            db.run(
                "INSERT INTO dnv_users (user_id, name, email, password_hash, role, is_verified, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)",
                [user_id, name, email, password_hash, role, is_verified, created_at],
                function (err: any)
                {
                    if (err)
                    {
    ...
    async editPassword(user: IUser): Promise<void>
    {
        return new Promise((resolve, reject) =>
        {
            ...
            db.run(
                "UPDATE dnv_users SET password_hash = ?, updated_at = ? WHERE user_id = ?",
    ...
    async recover(user: IUser): Promise<void>
    {
        return new Promise((resolve, reject) =>
        {
            ...
            db.run(
                "UPDATE dnv_users SET is_verified = ?, updated_at = ? WHERE user_id = ?",
                [is_verified, updated_at, user_id],
                function (err: any)
                {
                    if (err)
                    {
