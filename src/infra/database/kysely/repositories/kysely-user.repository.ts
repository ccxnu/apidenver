...
export class KyselyUserRepository implements UserRepository
{
    ...
    {
        const user = await this.database.selectFrom("user").where("email", "=", email).selectAll().executeTakeFirst();
    ...
    async findByUnique(unique: string): Promise<User | null>
    {
        const user = await this.database
            .selectFrom("user")
            .where((op) => op.or([op("cedula", "=", unique), op("email", "=", unique), op("username", "=", unique)]))
            .where("date_deleted", "is", null)
            .selectAll()
            .executeTakeFirst();
    ...
    {
        const { hasDisability, educationLevel, participation, jobPosition, role, deleted, page, perPage } = props;

        const skip = (page - 1) * perPage;

        let builder = this.database.selectFrom("user").where("date_deleted", deleted ? "is not" : "is", null);
        ...
        const people = await builder.limit(perPage).offset(skip).selectAll().execute();
    ...
    async editPassword(person: User): Promise<void>
    {
        ...
        await this.database.updateTable("user").set({ password, date_updated }).where("id", "=", id).executeTakeFirst();
    }
    ...
    async delete(person: User): Promise<void>
    {
        ...
        await this.database.updateTable("user").set({ date_deleted }).where("id", "=", id).executeTakeFirst();
    }
