...
export class KyselyCourseRepository implements CourseRepository
{
    ...
    async delete(data: Course): Promise<void>
    {
        ...
        await this.database.updateTable("course").set({ date_deleted }).where("id", "=", id).executeTakeFirst();
    }
    ...
    async findManyByFilters(props: FindManyByFiltersParams): Promise<PaginationData<CourseDetails[]>>
    {
        ...
        let builder = this.database.selectFrom("course").where("date_deleted", "is", null);
        ...
        const courses = await builder.limit(perPage).offset(skip).selectAll().execute();
