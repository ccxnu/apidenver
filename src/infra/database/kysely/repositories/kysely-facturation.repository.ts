...
export class KyselyFacturationRepository implements FacturationRepository
{
    ...
    async findManyByFilters(props: FindManyByFiltersParams): Promise<PaginationData<Facturation[]>>
    {
        ...
        let builder = this.database.selectFrom("facturation");
        ...
        const { total } = await builder.select((op) => op.fn.countAll<number>().as("total")).executeTakeFirst();

        const facturations = await builder.limit(perPage).offset(skip).selectAll().execute();
