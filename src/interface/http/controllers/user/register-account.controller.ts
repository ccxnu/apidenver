import { BadRequestException, Body, ConflictException, Controller, HttpCode, Ip, Post } from "@nestjs/common";
import { z } from "zod";
...
export class RegisterUserAccountController
{
    ...
    @Post()
    @HttpCode(201)
    async handle(@Body(bodyValidationPipe) body: RegisterBodySchema, @Ip() ip: string, @UserAgent() userAgent: string)
    {
