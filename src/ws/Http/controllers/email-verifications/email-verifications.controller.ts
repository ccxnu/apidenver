...
export class EmailVerificationsController
{
    ...
    create(@Body() createEmailVerificationDto: any)
    {
        ...
        {
            const sql =
                "INSERT INTO dnv_email_verifications (verification_id, user_id, token, expires_at, consumed_at) VALUES (?, ?, ?, ?, ?)";
    ...
    update(@Param("id") id: string, @Body() updateEmailVerificationDto: any)
    {
        ...
        return new Promise((resolve, reject) =>
        {
            ...
            const sql =
                "UPDATE dnv_email_verifications SET user_id = ?, token = ?, expires_at = ?, consumed_at = ? WHERE verification_id = ?";
