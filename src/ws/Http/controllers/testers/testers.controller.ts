...
export class TestersController
{
    ...
    create(@Body() createTesterDto: any)
    {
        ...
        {
            const sql =
                "INSERT INTO dnv_testers (tst_id, tst_full_name, tst_professional_license, tst_specialty, tst_email, tst_phone) VALUES (?, ?, ?, ?, ?, ?)";
            const { tst_id, tst_full_name, tst_professional_license, tst_specialty, tst_email, tst_phone } =
                createTesterDto;
            db.run(
                sql,
                [tst_id, tst_full_name, tst_professional_license, tst_specialty, tst_email, tst_phone],
                function (err: any)
                {
                    if (err)
                    {
    ...
    update(@Param("id") id: string, @Body() updateTesterDto: any)
    {
        ...
        return new Promise((resolve, reject) =>
        {
            ...
            const sql =
                "UPDATE dnv_testers SET tst_full_name = ?, tst_professional_license = ?, tst_specialty = ?, tst_email = ?, tst_phone = ? WHERE tst_id = ?";
            db.run(
                sql,
                [tst_full_name, tst_professional_license, tst_specialty, tst_email, tst_phone, id],
                function (err: any)
                {
                    if (err)
                    {
